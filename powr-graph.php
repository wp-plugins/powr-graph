<?php
     ...
     */
    /*
    Plugin Name: POWr Graph
    Plugin URI: http://www.powr.io
    Description: Graphs and charts for any website!  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Graph widget/////////////////////////////////
    class Powr_Graph extends WP_Widget{
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_graph',
                             __( 'POWr Graph' ),
                             array( 'description' => __( 'Graph by POWr.io') )
        );
      }
      //This prints the div
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-graph' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ?>
        <p>
          No need to edit here - just click the gears icon on your Graph.
        </p>
        <p>
    ...
    function register_powr_graph() {
      register_widget( 'Powr_Graph' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_graph' );
    //Create short codes for adding plugins anywhere:
    function powr_graph_shortcode( $atts ){
      if(isset($atts['label'])){
        $label = $atts['label'];
      }else{
        $label = '';
      }
      return "<div class='powr-graph' label='$label'></div>";
    }
    add_shortcode( 'powr-graph', 'powr_graph_shortcode' );
